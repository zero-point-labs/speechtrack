...
export const fileServiceSimple = {
  ...
  async uploadSmallFile(file, sessionId) {
    ...
    if (!response.ok) {
      ...
      if (response.status === 413) {
        throw new Error(`File too large for traditional upload (${this.formatFileSize(file.size)}). Please configure CORS on R2 bucket for direct uploads of large files.`);
  ...
  async uploadLargeFile(file, sessionId) {
    ...
    try {
      ...
      if (error.message.includes('CORS') || error.message.includes('fetch')) {
        console.log(`🚨 CORS Configuration Required: Please configure CORS on your R2 bucket`);
        console.log(`📋 Instructions: Check the R2_CORS_SETUP_INSTRUCTIONS.md file`);
      }
   ...
   * @param {string} fileName - The file name (optional, for extension checking)
  ...
  getFileCategory(mimeType, fileName = '') {
    ...
    // Document types - all return 'pdf' for backwards compatibility
   ...
   */
  isDocumentType(mimeType, fileName = '') {
    const documentMimeTypes = [
      'application/pdf',
      'application/msword',
      'application/vnd.openxmlformats-officedocument.wordprocessingml.document',
    ...
    return documentMimeTypes.includes(mimeType) || 
           documentExtensions.some(ext => fileName.toLowerCase().endsWith(ext));
  },
